...
namespace PetrolPump
{
    public partial class SummaryForm : Form
    {
        ...
        private void SummaryForm_Load(object sender, EventArgs e)
        {
            ...
            double BankOpening = Math.Round(Convert.ToDouble(Func.ScalarString("SELECT coalesce(sum(Amount),0) FROM (SELECT coalesce(sum(Amount),0) as 'Amount' FROM deposit where date(DateTime) < '" + date + "' union all SELECT coalesce(sum(Amount),0)*-1 as 'Amount' FROM withdraw where date(DateTime) < '" + date + "' union all SELECT coalesce(sum(Amount),0)*-1 FROM expense where type='Cheque' and date(DateTime) < '" + date + "' union all SELECT coalesce(sum(Amount),0) as 'Amount' FROM creditreceived where type='Cheque' and date(DateTime) < '" + date + "' union all SELECT coalesce(sum(Amount),0) as 'Amount' FROM vehiclecashreceived where type='Cheque' and date(DateTime) < '" + date + "') as temp")), 2);
            ...
            while (VCPayment.Read())
            {
                ...
                if (DGVSummary.Rows.Count > RowIndex)
                {
                    DGVSummary.Rows[RowIndex].Cells[3].Value = VCPayment[0].ToString();
                    DGVSummary.Rows[RowIndex].Cells[4].Value = Amount;
                }
                else
                {
            ...
            if (DGVSummary.Rows.Count > RowIndex)
            {
                DGVSummary.Rows[RowIndex].Cells[3].Value = "Total Sale";
                DGVSummary.Rows[RowIndex].Cells[4].Value = Sale;
            }
            else
            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            using (ExcelPackage package = new ExcelPackage(newFile))
            {
                ...
                for (int i = 0; i < DGVSummary.Rows.Count; i++)
                {
                    ...
                    {
                        if (DGVSummary.Rows[i].Cells[j].Value != "")
                ...
                if (CreditSaleRows > LineCreditSaleRows)
                {
                    ...
                    //worksheet.Cells[(29 + InventoryRows) + (MachineRows * 5) + CreditSaleRows, 7, (30 + InventoryRows) + (MachineRows * 5) + CreditSaleRows, 8].Merge = true;
                ...
                else
                {
                    ...
                    //worksheet.Cells[(29 + InventoryRows) + (MachineRows * 5) + LineCreditSaleRows, 7, (30 + InventoryRows) + (MachineRows * 5) + LineCreditSaleRows, 8].Merge = true;
